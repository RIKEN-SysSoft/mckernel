...
static const size_t contshift = PAGE_SHIFT + CONT_PAGE_SHIFT;
...
RUN_FUNC(TEST_SUITE, TEST_NUMBER)
{
	...
	// check
	{
		...
		tp_assert(info.present == 1 && info.pgsize == contpgsize, "madvise_addr - 1 error.");
		...
		tp_assert(info.present == 1 && info.pgsize ==  contpgsize, "madvise_end error.");
	}
