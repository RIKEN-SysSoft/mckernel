...
char* do_3xx(size_t shift, size_t cmpshift, int nr_cmppage,
	     ssize_t adjust_lower, ssize_t adjust_upper)
{
	...
	remap_length = cmppgsize + (cmppgsize * nr_cmppage) + cmppgsize;
	...
	from_end = (cmpaddr + cmppgsize * nr_cmppage) + adjust_upper;
	from_length = (unsigned long)from_end - (unsigned long)from_addr;

	to_addr = (void*)((align_up((unsigned long)remap_addr, cmppgsize) + cmppgsize) + (from_addr - cmpaddr));
	...
	to_end = to_addr + to_length;
	{
		...
		while (from < (unsigned long)from_end) {
			unsigned long next;
			tp_assert(i < sizeof(to_exp)/sizeof(to_exp[0]), "to_exp index error.");

			to_exp[i].addr = (void*)to;
			if ((unsigned long)to_exp[i].addr & (cmppgsize - 1)) {
				to_exp[i].pgsize = pgsize;
			} else {
				to_exp[i].pgsize = cmppgsize;
			}
	...
	// check
	{
		...
		if (cmpaddr <= from_addr) {
			info.present = 0;
			get_memory_info_self((unsigned long)from_addr - 1, &info);
			tp_assert(info.present == 1 && info.pgsize == pgsize, "from_addr - 1 error.");
		}
		...
		if (from_end <= (cmpaddr + cmppgsize * nr_cmppage)) {
			info.present = 0;
			get_memory_info_self((unsigned long)from_end, &info);
			tp_assert(info.present == 1 && info.pgsize == pgsize, "from_end error.");
		}

		// to
		for (i = 0; i < nr_to_exp; i++) {
			info.present = 0;
			get_memory_info_self((unsigned long)to_exp[i].addr, &info);
			tp_assert(info.present == 1 && info.pgsize == to_exp[i].pgsize, "to error.");
		}
	}
	return NULL;
}
