			 ...
			 size_t contpgsize, int nr_contiguous)
{
	...
	case CONT_PAGE_SIZE:
		pgsize = PAGE_SIZE;
		contshift = PAGE_SHIFT + CONT_PAGE_SHIFT;
		break;
	case CONT_LARGE_PAGE_SIZE:
		pgsize = LARGE_PAGE_SIZE;
		contshift = LARGE_PAGE_SHIFT + CONT_LARGE_PAGE_SHIFT;
		break;
	case CONT_LARGEST_PAGE_SIZE:
		pgsize = LARGEST_PAGE_SIZE;
		contshift = LARGEST_PAGE_SHIFT + CONT_LARGEST_PAGE_SHIFT;
		break;
	default:
		goto out;
	}
	...
	none_addr = mmap(NULL, none_length,
			 PROT_NONE,
			 MAP_PRIVATE | MAP_ANONYMOUS | MAP_HUGETLB | (contshift << MAP_HUGE_SHIFT),
			 -1, 0);
	...
	res = mprotect(aligned_addr, contpgsize * nr_contiguous, PROT_READ|PROT_WRITE);
	if (res == -1) {
	...
	*(hi_normal_page) = 'z';
	...
	// join
	res = mprotect(lo_normal_page, pgsize + (contpgsize * nr_contiguous) + pgsize, PROT_READ|PROT_WRITE);
	if (res == -1) {
	...
	for (i = 0; i < nr_contiguous; i++) {
		check_page_size((unsigned long)aligned_addr + contpgsize * i, contpgsize);
	}
		       ...
		       size_t contpgsize, int nr_contiguous)
{
	...
	// reserve
	shm_length = (contpgsize + (contpgsize * nr_contiguous) + contpgsize) + contpgsize;
	if (huge) {
		shmid = shmget(IPC_PRIVATE, shm_length, IPC_CREAT | SHM_R | SHM_HUGETLB | (contshift << MAP_HUGE_SHIFT));
	} else {
	...
	// map
	res = mprotect(lo_page, contpgsize + (contpgsize * nr_contiguous) + contpgsize, PROT_READ|PROT_WRITE);
	if (res == -1) {
