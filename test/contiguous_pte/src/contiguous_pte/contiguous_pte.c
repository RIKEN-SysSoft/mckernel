...
void* map_contiguous_pte(size_t* length,
			 ...
			 size_t cmppgsize, int nr_compound)
{
	char* ptr = MAP_FAILED;
	...
	case CONT_PAGE_SIZE:
		pgsize = PAGE_SIZE;
		cmpshift = PAGE_SHIFT + CONT_PAGE_SHIFT;
		break;
	case CONT_LARGE_PAGE_SIZE:
		pgsize = LARGE_PAGE_SIZE;
		cmpshift = LARGE_PAGE_SHIFT + CONT_LARGE_PAGE_SHIFT;
		break;
	case CONT_LARGEST_PAGE_SIZE:
		pgsize = LARGEST_PAGE_SIZE;
		cmpshift = LARGEST_PAGE_SHIFT + CONT_LARGEST_PAGE_SHIFT;
		break;
	default:
		goto out;
	}
	...
	// reserve
	none_length = cmppgsize + (cmppgsize * nr_compound) + cmppgsize;
	...
	aligned_addr += cmppgsize;
	res = mprotect(aligned_addr, cmppgsize * nr_compound, PROT_READ|PROT_WRITE);
	if (res == -1) {
		fprintf(stderr, "mprotect(aligned) error.\n");
		goto out;
	}
	for (i = 0; i < nr_compound; i++) {
		*(aligned_addr + cmppgsize * i) = 'z';
	}

	// map neighbor
	lo_normal_page = aligned_addr - pgsize;
	hi_normal_page = aligned_addr + cmppgsize * nr_compound;
	...
	*(hi_normal_page) = 'z';
	...
	// join
	res = mprotect(lo_normal_page, pgsize + (cmppgsize * nr_compound) + pgsize, PROT_READ|PROT_WRITE);
	if (res == -1) {
	...
	for (i = 0; i < nr_compound; i++) {
		check_page_size((unsigned long)aligned_addr + cmppgsize * i, cmppgsize);
	}
		       ...
		       size_t cmppgsize, int nr_compound)
{
	...
	// reserve
	shm_length = (cmppgsize + (cmppgsize * nr_compound) + cmppgsize) + cmppgsize;
	if (huge) {
		shmid = shmget(IPC_PRIVATE, shm_length, IPC_CREAT | SHM_R | SHM_HUGETLB | (cmpshift << MAP_HUGE_SHIFT));
	} else {
		shmid = shmget(IPC_PRIVATE, shm_length, IPC_CREAT | SHM_R);
	}
	if (shmid == -1) {
	...
	// calc contiguous
	aligned_addr = (void*)align_up((unsigned long)shm_addr, cmppgsize);
	aligned_addr += cmppgsize;

	// calc neighbor
	lo_page = aligned_addr - cmppgsize;
	hi_page = aligned_addr + cmppgsize * nr_compound;

	// map
	res = mprotect(lo_page, cmppgsize + (cmppgsize * nr_compound) + cmppgsize, PROT_READ|PROT_WRITE);
	if (res == -1) {
	...
	*(hi_page) = 'z';
	...
	//check
	check_page_size((unsigned long)lo_page, cmppgsize);
	for (i = 0; i < nr_compound; i++) {
		check_page_size((unsigned long)aligned_addr + cmppgsize * i, cmppgsize);
	}
	check_page_size((unsigned long)hi_page, cmppgsize);
