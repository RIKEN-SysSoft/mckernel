...
# Usage: 008.sh -m <Use Mckernel> -s <mcstop> -r <mcreboot> -g -S <server node> -C <client node> -l <loop count>
while getopts srgml:S:C: OPT
do
        case ${OPT} in
            s) stop=1
                ;;
...
if [ ${go} -eq 1 ]; then
	...
	rm -f ${HOME}/tmp/psm2-demo-server-epid-*

	# Note that Linux and McKernel cannot communicate via shared memory
	# with Mckernel hfi driver
	ssh $clientnode PSM2_RCVTHREAD=0 PMI_RANK=0 DISABLE_UTI=0 \
	    $mcexec $mcexecopt taskset -c 2 ${test_dir}/$exe --ppn 1 &
	ssh $servernode PSM2_RCVTHREAD=0 PMI_RANK=1 DISABLE_UTI=0 \
	    $mcexec $mcexecopt taskset -c 3 ${test_dir}/$exe --ppn 1
