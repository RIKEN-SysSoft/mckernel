...
static int parent_func(pid_t cpid, unsigned int vq)
{
	...
	char *l_rd_buf = NULL;
	sve_regs_t cmp_buf;
	user_sve_regs_t rd_buf;
	size_t align_half_regsz = (sizeof(sve_regs_t) / 2 + 15) & ~15UL;
	...
	if (rd_buf.header.vl != 0 || rd_buf.header.max_vl != 0 || rd_buf.header.flags != 0) {
	...
	/* case 5: iov_len is sizeof(struct user_sve_header) + sizeof(sve_regs_t) / 2 */
	printf("check 5: iov_len header + (registerarea size / 2) (16 byte align)\n");
	gen_test_sve(&cmp_buf, vq);
	memset((char *)&cmp_buf + align_half_regsz, 0, sizeof(sve_regs_t) - align_half_regsz);
	...
	iov.iov_len = ((SVE_PT_SIZE(SVE_VQ_MAX, SVE_PT_REGS_SVE) + 15) / 16 * 16) + 16;
	...
	/* compare */
	iov.iov_base = l_rd_buf;
	if (sve_compare(&cmp_buf, l_rd_buf + sizeof(struct user_sve_header), vq)) {
		printf("parent-process compare failed.\n");
		free(iov.iov_base);
		goto free_cont;
	}
