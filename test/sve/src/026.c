...
{
	int ret = -1;
	const unsigned int bf_vq = sve_vq_from_vl(vl);		   /* before SETREGS VQ */
	const unsigned int af_vq = sve_vq_from_vl(gen_set_vl(vl)); /* after  SETREGS VQ */
	typedef struct fpsimd_sve_state(bf_vq) bf_sve_regs_t;
	typedef struct fpsimd_sve_state(af_vq) af_sve_regs_t;
	bf_sve_regs_t bf_rd_buf;
	af_sve_regs_t cmp_buf, af_rd_buf;
...
static int parent_func(pid_t cpid, const unsigned int vl)
{
	int ret = -1;
	const unsigned int af_vq = sve_vq_from_vl(gen_set_vl(vl)); /* after  SETREGS VQ */
	typedef struct user_fpsimd_sve_state(af_vq) af_user_sve_regs_t;
	af_user_sve_regs_t wr_buf;
	struct iovec iov;
