...
/* TP# 4 */	"Use execve() takeover registers check. (execve target is #1 testcase)",
...
/* TP#11 */	"Preservation check signalhsndler(use sigaltstack) before and after.",
...
/* TP#15 */	"Context switch in the same core check and SIGSTOP -> SIGCONT restart check.(need run on background '&')",
...
/* TP#22 */	"Use execve() VL check if setting INHERIT flags. (execve target is #1 testcase)",
...
/* TP#29 */	"Use execve() VL check if not setting INHERIT flags. (execve target is #1 testcase)",
/* TP#30 */	"When SVE is enable, ptrace(SETREGSET + NT_ARM_SVE + SVE_PT_REGS_FPSIMD), regs fpsimd struct check.",
/* TP#31 */	"When SVE is enable, ptrace(SETREGSET + NT_ARM_SVE + SVE_PT_REGS_FPSIMD), regs sve struct check.",
...
/* TP#36 */	"It becomes SIGSEGV when changing and using SVE-VL during signalhandler execution.",
/* TP#37 */	"Confirmation of execve() operation when VL is set in INHERIT->ONEXEC order.",
/* TP#38 */	"Confirmation of execve() operation when VL is set in ONEXEC->INHERIT order.",
/* TP#39 */	"Confirmation of execve() operation when ONEXEC and INHERIT are set at the same time.",
/* TP#40 */	"Coredump SVE register output check.",
};

struct test_case {
	int (*func)(int, unsigned int, unsigned int, int, char **);
};
...
{
	int i = 0;
	int tp_count = sizeof(usage_messages) / sizeof(usage_messages[0]) - 1;
...
int main(int argc, char *argv[])
{
	...
	int usage_count = sizeof(usage_messages) / sizeof(usage_messages[0]);
	int tp_count = sizeof(test_cases) / sizeof(test_cases[0]);
