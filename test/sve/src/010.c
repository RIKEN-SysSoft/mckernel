...
static int child_func(unsigned int vq)
{
	...
	memset(&zero_buf, 0, sizeof(sve_regs_t));
...
static int parent_func(pid_t cpid, unsigned int vq)
{
	...
	/* case 9: iov_len is sizeof(struct user_sve_header) + sizeof(sve_regs_t) / 2 */
	printf("check  9: iov_len header + (registerarea size / 2) (16 byte align)\n");
	memset(&wr_buf, 0, sizeof(wr_buf));
