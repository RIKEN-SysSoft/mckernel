...
int main(void)
{
	...
	// mprotec PROT_WRITE to ptr1
	rc = mprotect(ptr1, pgsize, PROT_WRITE);
	if (rc != 0) {
		perror("mprotect");
		ret = -1;
		goto out;
	}
	printf("** mprotect PROT_WRITE: Succeed\n");

	// Check prot after mprotect
	rc = syscall(899, ptr1);
	if (rc == DEF_PROT) {
		printf("[OK] after mprotect : %d\n", rc);
		printf(" (McKernel doesn't change host side vma's prot,\n");
		printf("  when mprotect specify PROT_WRITE)\n");
	}
	else {
