...
echo "** xpem_attach to Huge mapped memory range"
...
do
	...
	XPMEM_PGSIZE=`grep -o "XPMEM_PAGE_FAULT.*${XPMEM_ADDR}.*" ${log_file} | awk '{ print $5; }'`
...
do
	...
	log_file="./C${issue}T${tid}_${pgshift}.log"
	echo pageshift: ${pgshift}
	${MCEXEC} ./huge_page_xpmem ${pgshift} 1 4096 > ${log_file}
	${IHKOSCTL} 0 kmsg >> ${log_file}
...
ng=0
echo "*** C${issue}T${tid} start *******************************"
echo "** xpem_attach to small mapped memory range"
${IHKOSCTL} 0 clear_kmsg
