			  ...
			  unsigned int flags)
{
	...
	ovl_reset_ovl_entry(&poe, dentry->d_parent);

	upperdir = ovl_upperdentry_dereference(poe);
	if (upperdir) {
		this = ovl_lookup_real(upperdir, &dentry->d_name, opt);
		err = PTR_ERR(this);
		if (IS_ERR(this))
			goto out;
	...
	for (i = 0; !upperopaque && i < poe->numlower; i++) {
		...
		this = ovl_lookup_real(lowerpath.dentry, &dentry->d_name, opt);
		err = PTR_ERR(this);
		if (IS_ERR(this)) {
			/*
...
static int ovl_lower_dir(const char *name, struct path *path, long *namelen,
			 int *stack_depth, bool *remote, unsigned opt)
{
	int err;
