...
#ifdef DEBUG_UTI
    //fprintf(stderr, "%s: wp=%p,mck_tid=%d,key=%lx,pid=%d,tid=%d,uti_clv=%lx,pattr=%lx\n", __FUNCTION__, uti_desc->wp, uti_desc->mck_tid, uti_desc->key, uti_desc->pid, uti_desc->tid, uti_desc->uti_clv, pattr);
#endif
		...
		case __NR_exit_group:
			...
			if(w.sr.number == __NR_exit_group){
				...
				if(isatty(2)){
					if(sig){
						if(!ischild)
							fprintf(stderr, "%s: Terminated by signal %d, tid=%d\n", __FUNCTION__, sig, my_thread->tid);
					}
					else if(term)
