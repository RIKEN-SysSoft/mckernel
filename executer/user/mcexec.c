...
static inline void dirent_set_d_off(int sysnum, void *_dirp,
				    unsigned long d_off)
{
...
{
	void *dirp = NULL;
	void *linux_dirp_iter, *mck_dirp_iter, *dirp_iter;
...
linux_again:
	...
	/* fix d_off */
	for (pos = 0; pos < mck_len + linux_len;
	     pos += dirent_reclen(sysnum, dirp_iter)) {
		dirp_iter = _dirp + pos;
		reclen = dirent_reclen(sysnum, dirp_iter);
		dirent_set_d_off(sysnum, dirp_iter, pos + reclen);
	}
