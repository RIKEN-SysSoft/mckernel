...
{
	void *mck_dirp = NULL, *linux_dirp = NULL;
	void *linux_dirp_iter, *mck_dirp_iter;
	int ret;
	int mck_ret = 0, old_mck_ret = 0, pos;
	int linux_ret = 0, old_linux_ret = 0, mcpos;
...
mck_again:
	ret = syscall(sysnum, ofd->getdents_fd, mck_dirp, count - mck_ret);
	if (ret < 0) {
		ret = -errno;
		goto err;
	}
	...
	if (ret > 0) {
		...
		memcpy(ofd->mck_dirents + ofd->mck_dirents_size, mck_dirp, ret);
		ofd->mck_dirents_size += ret;
	}
...
linux_again:
	ret = syscall(sysnum, ofd->linux_fd, linux_dirp, count - linux_ret);
	if (ret < 0) {
		ret = -errno;
	...
	for (pos = 0; pos < ret;) {
		linux_dirp_iter = linux_dirp + pos;
		...
		if (overlay_blacklist(ofd_path)) {
			__dprintf("blacklisted %s\n", ofd_path);
			memmove(linux_dirp + pos,
				linux_dirp + pos + reclen,
				ret - pos - reclen);
			ret -= reclen;
			continue;
		}
	 ...
	 */
	if (old_linux_ret != linux_ret) {
		old_linux_ret = linux_ret;
		goto linux_again;
	}

	/* concatinate cached upper and lower and lseek */
