...
static inline void numa_local(__cpu_set_unit *localset,
			      unsigned long *nodemask, int nonlocal)
{
	...
	for (i = 0; i < nnodes; i++) {
		...
		for (j = 0; j < ncpu; j++) {
			...
			int cpu_isset_local = localset[jndx] &
				(1UL << jpos);
...
#define	MCEXEC_MAX_STACK_SIZE	(16 * 1024 * 1024)	/* 1 GiB */
	...
	/* Fujitsu TCS specific: mempolicy */
		...
		if (!strncmp(mpol, "localalloc", 10)) {
			...
			numa_local(desc->cpu_set, desc->mpol_nodemask, 0);
		}
		else if (!strncmp(mpol, "interleave_nonlocal", 19)) {
			desc->mpol_mode = MPOL_INTERLEAVE;
			numa_local(desc->cpu_set, desc->mpol_nodemask, 1);
		}
