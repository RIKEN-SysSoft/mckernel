...
void overlay_addfd(int fd, const char *path)
{
	...
	if (strncmp(real_path, "/sys/", 5) && strncmp(real_path, "/proc/", 6))
		return;
	...
	ofd->dirents = NULL;
	ofd->dirents_size = 0;
	ofd->pathlen = snprintf(ofd->path, PATH_MAX, "%s", real_path);
...
void overlay_delfd(int fd)
{
	...
	/* should be empty most of the time, spare locking if so */
	if (list_empty(&overlay_fd_list))
		return;
...
int overlay_getdents(int sysnum, int fd, void *_dirp, unsigned int count)
{
	...
	if (&ofd->link == &overlay_fd_list || ofd->linux_fd == -1) {
		ret = syscall(sysnum, fd, _dirp, count);
		if (ret == -1)
			ret = -errno;
	}
	if (&ofd->link == &overlay_fd_list || ret < 0)
		return ret;
