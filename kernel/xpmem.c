	...
	int page_in_remote)
{
	...
	if (att_pte && !pgsize_is_contiguous(att_pgsize)) {
		kprintf("%s: att_pgaddr: %lx, att_pgsize: %lx, "
			"seg_vaddr: %lx, seg_pgsize: %lx, seg_phys: %lx\n",
			__func__, att_pgaddr, att_pgsize, seg_vaddr,
			seg_pgsize, seg_phys);
	...
	size_t *pgsize)
{
	...
	XPMEM_DEBUG("vaddr: %lx, pgshift: %d, base: %lx, size: %lx, "
		    "p2align: %x\n",
		    vaddr, pgshift, (unsigned long)base, size, p2align);
	if (pte) {
		*pgsize = size;
	}
	else {
