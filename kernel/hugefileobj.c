				...
				unsigned long *pflag, uintptr_t virt_addr)
{
	...
	if (!obj->pages[pgind]) {
		obj->pages[pgind] = ihk_mc_alloc_aligned_pages_user(npages, p2align,
	...
	if (obj->pages) {
		...
		for (i = 0; i < obj->nr_pages; ++i) {
			if (obj->pages[i]) {
				...
				dkprintf("%s: obj: 0x%lx, freed page at ind: %d\n",
						__func__, obj, i);
			}
		}
		       ...
		       int *pgshiftp, uintptr_t virt_addr)
{
	...
	ret = arch_get_smaller_page_size(NULL, obj->pgsize + 1, NULL,
					   &p2align);
	if (ret)
		return ret;
	...
	if (obj->nr_pages < nr_pages) {
		void **pages = kmalloc(nr_pages * sizeof(void *), IHK_MC_AP_NOWAIT);
		if (!pages) {
			ret = -ENOMEM;
			goto out;
		}
		...
		if (obj->nr_pages) {
			memcpy(pages, obj->pages, obj->nr_pages * sizeof(void *));
		}
