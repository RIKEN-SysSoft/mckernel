...
straight_out:
	...
	if (flags & MAP_FIXED) {
		...
		if (vrf0 & VR_XPMEM) {
			/* Try to obtain the same page layout with
			 * possibly mixed page sizes as the segment
			 */
...
out:
	...
	if (cpu_local_var(current)->profile) {
	 ...
	 if(range->memobj && range->memobj->flags & MF_XPMEM) {
		 kprintf("%s: error: %lx, range->straight_start: %lx, addr: %lx\n",
			 __func__, error, range->straight_start, addr);
	 }
