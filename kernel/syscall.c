...
static void send_syscall(struct syscall_request *req, int cpu, int pid, struct syscall_response *res)
{
	...
	kprintf("send syscall, nr: %d, pid: %d, resp_pa=%lx\n", req->number, packet.pid, packet.resp_pa);
			  ...
			  void *_linux_clock_gettime)
{
	...
	//uti_dkprintf("%s: n=%d, uaddr=%p, op=%x, val=%x, utime=%p, uaddr2=%p, val3=%x, clv=%lx, resp=%p, wait_event=%p, printk=%p, clock_gettime=%p, fshared=%d\n", __FUNCTION__, n, arg0, arg1, arg2, arg3, arg4, arg5, _uti_clv, uti_futex_resp, _linux_wait_event, _linux_printk, _linux_clock_gettime, fshared);
