...
int free_process_memory_range(struct process_vm *vm, struct vm_range *range)
{
	...
	if (!(range->flag & (VR_REMOTE | VR_IO_NOCACHE | VR_RESERVED))) {
		...
		error = ihk_mc_pt_free_range(vm->address_space->page_table, vm,
				...
				IHK_MC_DONT_FLUSH_TLB : IHK_MC_FLUSH_TLB);
		if (range->memobj) {
...
straight_out:
	...
	if (range->straight_start) {
		...
		kprintf("%s: straight range 0x%lx @ straight 0x%lx physical memory freed\n",
		...
		unsigned long start, unsigned long end, int *ro_freedp)
{
	...
	dkprintf("remove_process_memory_range(%p,%lx,%lx)\n",
			vm, start, end);
	/* Convert to real virtual address for straight mapping */
	if (vm->proc->straight_va &&
		...
		kprintf("%s: straight range converted from 0x%lx:%lu -> 0x%lx:%lu\n",
				__FUNCTION__,
				start, len,
