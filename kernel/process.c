...
set_timer()
{
	...
	if (v->runq_len > 1 || v->current->itimer_enabled) {
		if (!cpu_local_var(timer_enabled)) {
			kprintf("%s: INFO interval timer activated,cpu=%d,pid=%d,tid=%d,runq_len=%d,itimer_enabled=%d\n", __func__, ihk_mc_get_processor_id(), v->current->proc->pid, v->current->tid, v->runq_len, v->current->itimer_enabled);
