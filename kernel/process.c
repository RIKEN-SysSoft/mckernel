...
void set_timer(int runq_locked)
{
	...
	if (num_running > 1 || v->current->itimer_enabled) {
		if (!cpu_local_var(timer_enabled)) {
			dkprintf("%s: INFO interval timer activated,pid=%d,tid=%d,runq_len=%d\n", __func__, v->current->proc->pid, v->current->tid, v->runq_len);
		...
		if (cpu_local_var(timer_enabled)) {
			dkprintf("%s: INFO interval timer deactivated,pid=%d,tid=%d,runq_len=%d\n", __FUNCTION__, v->current->proc->pid, v->current->tid, v->runq_len);
			lapic_timer_disable();
		...
		int valid_states, int runq_locked)
{
	...
	if (thread->status & valid_states) {
		...
		/* Call schedule in interrupt_exit() */
		v->flags |= CPU_FLAG_NEED_RESCHED;
	}
	else {
		status = -EINVAL;
