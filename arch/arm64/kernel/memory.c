...
uint64_t ihk_mc_pt_virt_to_pagemap(struct page_table *pt, unsigned long virt)
{
	...
	if (ptl3_type_block(ptep)) {
		...
		if (pte_is_compound(ptep)) {
			size = PTL3_CONT_SIZE;
			mask = PTL3_CONT_MASK;
			shift = PTL3_SHIFT + PTL3_CONT_SHIFT;
		} else {
			size = PTL3_SIZE;
			mask = PTL3_MASK;
			shift = PTL3_SHIFT;
	...
	if (ptl2_type_block(ptep)) {
		...
		if (pte_is_compound(ptep)) {
			size = PTL2_CONT_SIZE;
			mask = PTL2_CONT_MASK;
			shift = PTL2_SHIFT + PTL2_CONT_SHIFT;
		} else {
			size = PTL2_SIZE;
			mask = PTL2_MASK;
			shift = PTL2_SHIFT;
	...
	if (pte_is_compound(ptep)) {
		size = PTL1_CONT_SIZE;
		mask = PTL1_CONT_MASK;
		shift = PTL1_SHIFT + PTL1_CONT_SHIFT;
	} else {
		size = PTL1_SIZE;
		mask = PTL1_MASK;
		shift = PTL1_SHIFT;
		...
		uintptr_t *basep, size_t *sizep, int *p2alignp)
{
	...
	ptep = NULL;
	if (!pgshift) {
		pgshift = FIRST_LEVEL_BLOCK_SUPPORT ? (PTL3_SHIFT + PTL3_CONT_SHIFT) : (PTL2_SHIFT + PTL2_CONT_SHIFT);
	}

	ptep = ptl4_offset(tt, virt);
	if (ptl4_null(ptep)) {
		if (pgshift > (PTL3_SHIFT + PTL3_CONT_SHIFT)) {
			pgshift = PTL3_SHIFT + PTL3_CONT_SHIFT;
	...
	if (ptl2_null(ptep)) {
		if (pgshift >= (PTL2_SHIFT + PTL2_CONT_SHIFT)) {
			pgshift = PTL2_SHIFT + PTL2_CONT_SHIFT;
	...
	if (ptl1_type_page(ptep)) {
		if (ptl1_is_compound(ptep) &&
		    pgshift >= (PTL1_SHIFT + PTL1_CONT_SHIFT)) {
			pgshift = PTL1_SHIFT + PTL1_CONT_SHIFT;
		} else {
			pgshift = PTL1_SHIFT;
		}
	...
	if (pgshift >= (PTL1_SHIFT + PTL1_CONT_SHIFT)) {
		pgshift = PTL1_SHIFT + PTL1_CONT_SHIFT;
	} else {
		pgshift = PTL1_SHIFT;
	}
		...
		uintptr_t end)
{
	...
	if (__page_offset(base, PTL1_CONT_SIZE) == 0) { //check head pte
		uintptr_t next_addr = ALIGN_UP(base + 1, PTL1_CONT_SIZE);
		if (end < next_addr) {
			next_addr = end;
		}
	...
	if (ptl_null(ptep, level) || (args->overwrite && ptl_type_page(ptep, level))) {
		...
		if (level == 2 || (level == 3 && FIRST_LEVEL_BLOCK_SUPPORT)) {
			if ((start <= base) && ((base + tbl.pgsize) <= end)
				...
				if (__page_offset(base, tbl.cmp_pgsize) == 0) { //check head pte
					uintptr_t next_addr = ALIGN_UP(base + 1, tbl.cmp_pgsize);
					if (end < next_addr) {
						next_addr = end;
					}

					// update attribute
					if (__page_offset(phys | next_addr, tbl.cmp_pgsize) == 0) {
		...
		int *p2alignp)
{
	...
	if (0) {
	...
	else if ((cursize > PTL3_CONT_SIZE) && CONFIG_ARM64_PGTABLE_LEVELS > 2) {
		newsize = PTL3_CONT_SIZE;
		p2align = (PTL3_SHIFT + PTL3_CONT_SHIFT) - PTL1_SHIFT;
	}
		...
		p2align = (PTL2_SHIFT + PTL2_CONT_SHIFT) - PTL1_SHIFT;
	}
		...
		p2align = (PTL1_SHIFT + PTL1_CONT_SHIFT) - PTL1_SHIFT;
	}
