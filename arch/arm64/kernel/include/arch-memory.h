...
# define __PTL4_CONT_SIZE  (UL(1) << (__PTL4_SHIFT + __PTL4_CONT_SHIFT))
# define __PTL3_CONT_SIZE  (UL(1) << (__PTL3_SHIFT + __PTL3_CONT_SHIFT))
# define __PTL2_CONT_SIZE  (UL(1) << (__PTL2_SHIFT + __PTL2_CONT_SHIFT))
# define __PTL1_CONT_SIZE  (UL(1) << (__PTL1_SHIFT + __PTL1_CONT_SHIFT))
...
# define __PTL4_CONT_COUNT  (UL(1) << __PTL4_CONT_SHIFT)
# define __PTL3_CONT_COUNT  (UL(1) << __PTL3_CONT_SHIFT)
# define __PTL2_CONT_COUNT  (UL(1) << __PTL2_CONT_SHIFT)
# define __PTL1_CONT_COUNT  (UL(1) << __PTL1_CONT_SHIFT)
...
static inline int pte_is_type_page(const pte_t *ptep, size_t pgsize)
{
	...
	else if (PTL1_SIZE == pgsize || PTL1_CONT_SIZE == pgsize) {
		// check D_PAGE
...
static inline int pte_is_compound(pte_t *ptep)
{
	return !!(*ptep & PTE_CONT);
}

static inline int pgsize_is_compound(size_t pgsize)
{
	int ret = 0;
...
static inline size_t tbllv_to_cmppgsize(int level)
{
	size_t pgsize = 0;
	switch (level) {
	case 4:
...
static inline int tbllv_to_cmppgshift(int level)
{
	int ret = 0;
	switch (level) {
	case 4:
		if (CONFIG_ARM64_PGTABLE_LEVELS > 3) {
			ret = PTL4_SHIFT + PTL4_CONT_SHIFT;
		}
		break;
	case 3:
		if (CONFIG_ARM64_PGTABLE_LEVELS > 2) {
			ret = PTL3_SHIFT + PTL3_CONT_SHIFT;
		}
		break;
	case 2:
		ret = PTL2_SHIFT + PTL2_CONT_SHIFT;
		break;
	case 1:
		ret = PTL1_SHIFT + PTL1_CONT_SHIFT;
		break;
	}
	return ret;
}

static inline pte_t* get_compound_head(pte_t *__ptep, size_t __pgsize)
{
	unsigned long align;
	int shift = 0;
...
static inline pte_t* get_compound_tail(pte_t *__ptep, size_t __pgsize)
{
	unsigned long align;
	int shift = 0;
	switch (pgsize_to_tbllv(__pgsize)) {
...
static inline int isolation_ptes(pte_t *ptep, size_t pgsize)
{
	int ret;
	pte_t *head = get_compound_head(ptep, pgsize);
	pte_t *tail = get_compound_tail(ptep, pgsize);
	pte_t *ptr;
